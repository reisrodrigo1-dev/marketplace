...
const LawyerAppointments = () => {
  ...
  const [pageFilter, setPageFilter] = useState('todos'); // todos, ou ID específico da página
  ...
  // Obter páginas únicas dos agendamentos
  const getUniquePagesFromAppointments = () => {
    const pages = new Map();
    
    appointments.forEach(appointment => {
      if (appointment.paginaOrigem && appointment.paginaOrigem.id) {
        pages.set(appointment.paginaOrigem.id, {
          id: appointment.paginaOrigem.id,
          nomePagina: appointment.paginaOrigem.nomePagina,
          tipoPagina: appointment.paginaOrigem.tipoPagina || 'advogado'
        });
      }
    });
  ...
  // Função para contar agendamentos por filtro de data (considerando o filtro de status atual)
  ...
  const filteredAppointments = appointments.filter(appointment => {
    ...
    if (pageFilter !== 'todos' && appointment.paginaOrigem?.id !== pageFilter) {
      return false;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {getUniquePagesFromAppointments().length > 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-8">
            <div className="px-6 py-4 border-b border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-lg font-bold text-gray-900">{appointments.length}</div>
                    <div className="text-sm text-gray-600">Todas as Páginas</div>
                  </div>
                </div>
                
                {getUniquePagesFromAppointments().map((page) => {
                  const pageAppointments = appointments.filter(apt => apt.paginaOrigem?.id === page.id);
                  return (
                    <div
                      key={page.id}
                    ...
                    >
                      <div className="text-center">
                        ...
                        <div className="text-lg font-bold text-gray-900">{pageAppointments.length}</div>
              ...
              <div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                  ...
                  {selectedAppointment.paginaOrigem && (
                    <p><strong>Página de Origem:</strong> 
                      <span className="ml-2">
                        {selectedAppointment.paginaOrigem.nomePagina}
                        <span className={`ml-2 px-2 py-1 text-xs font-medium rounded-full ${
                          selectedAppointment.paginaOrigem.tipoPagina === 'escritorio' 
                            ? 'bg-blue-100 text-blue-800' 
                            : 'bg-purple-100 text-purple-800'
                        }`}>
                          {selectedAppointment.paginaOrigem.tipoPagina === 'escritorio' ? 'Escritório' : 'Advogado'}
                        </span>
                      </span>
                    </p>
                  )}
