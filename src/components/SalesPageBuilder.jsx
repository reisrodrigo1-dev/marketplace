...
const SalesPageBuilder = ({ onBack, onPageCreated, onPageUpdated, editingPage = null }) => {
  ...
  // Handler genérico para campos controlados
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              {[1, 2, 3, 4, 5].map((step, idx) => (
                <React.Fragment key={step}>
                  <div className={`flex flex-col items-center w-1/5`}>
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full text-base font-bold border-2 transition-all duration-200 ${
                      step <= currentStep
                    ...
                    <span className={`mt-2 text-xs font-medium ${step === currentStep ? 'text-blue-700' : 'text-gray-600'}`}>{
                      [
                        'Básico',
                        'Cor',
                        'Textos',
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          {/* Removido bloco duplicado da etapa de cor e botões de navegação */}
          {/* Removido bloco duplicado da etapa 3 (Textos da Página) */}
          {/* Removido bloco duplicado das etapas 4 e 5 (Produtos e Preview) e botões de navegação */}
        </div>
      </div>
    </div>
  );
